...
import pickle
from setup_cifar100 import setup_cifar100
import pandas as pd
...
if args.opening_layer:
    if args.loss == 't_cross_entropy':
        net = torch.nn.Sequential(View((-1, 32 * 3, 32)),
                                  ...
                                  tLinearLayer(args.width, n_classes, dim3, M=M, activation=None)
                                  ).to(device)
        ...
        net = torch.nn.Sequential(View((-1, 32 * 3, 32)),
                                  ...
                                  LinearLayer(args.width * dim3, n_classes, activation=None, bias=args.bias)
                                  ).to(device)
    ...
    if args.loss == 't_cross_entropy':
        net = torch.nn.Sequential(View((-1, w, 32)),
                                  ...
                                  tHamiltonianResNet(w, w + args.add_width_hamiltonian, dim3, M,
                                                     depth=args.depth, h=args.h_step, activation=torch.nn.Tanh()),
                                  tLinearLayer(w, n_classes, dim3, M=M, activation=None)
                                  ).to(device)
        loss = tCrossEntropyLoss(M=M)
    else:
        net = torch.nn.Sequential(View((-1, w, 32)),
                                  ...
                                  Permute((1, 0, 2)),
                                  View((-1, w * dim3)),
                                  LinearLayer(w * dim3, n_classes, activation=None, bias=args.bias)
                                  ).to(device)
        loss = torch.nn.CrossEntropyLoss()
