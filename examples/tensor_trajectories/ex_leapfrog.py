...
if args.loss == 't_cross_entropy':
    net = torch.nn.Sequential(
        View((-1, 1, 3)),
        Permute((1, 0, 2)),
        tHamiltonianResNet(1, width=1, dim3=dim3, M=M, depth=32, h=args.h, activation=torch.nn.Tanh()),
...
sPath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'experiments', 'leapfrog')
filename = 'tmp.log'
makedirs(sPath)
